/*
 * This function draws the Set Clock Screen date containers, and populates them with the last querried date obtained from the RTC clock or the last time setted by the user to be applied to the RTC clock.
 * The last parameter received in this function, the updateAll parameter, indicates wheter to update the entire date table, if equal to 0, or just a specific time container, being, left to right, 1 to 6 each container.
 */

void drawDateContainers_SetClockScreen(byte daysTens, byte daysUnits, byte monthsTens, byte monthsUnits, byte yearsTens, byte yearsUnits, byte updateAll){
